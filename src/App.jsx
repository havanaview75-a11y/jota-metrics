    ...
    <div className="rounded-[20px] border border-[#243041] bg-[#111827] p-4 shadow-[0_10px_30px_rgba(0,0,0,0.25)]">
      ...
      <div className="flex h-[110px] items-end gap-2">
        {data.map((item) => {
          ...
          return (
            <div key={`${item.label}-${item.pnl}`} className="flex flex-1 flex-col items-center gap-2">
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        ...
        <div className="text-right">
          <div className="text-[12px] text-[#dbe5f3]">{getRangeLabel(trades)}</div>
...
function RecordCard({ trade, onDelete, onEdit, isAdmin }) {
  ...
  <>
    <button
      ...
      className="ml-auto rounded-full border border-[#1e3a5f] bg-[#10253f] px-3 py-1 text-[11px] text-[#93c5fd] transition hover:opacity-90"
    >
      Edit
    </button>
    ...
    <button
  onClick={() => {
    console.log("Delete trade id:", trade.id);
    onDelete(trade.id);
  }}
  className="rounded-full border border-[#5b2121] bg-[#311616] px-3 py-1 text-[11px] text-[#fca5a5] transition hover:opacity-90"
>
  Delete
</button>
  </>
      ...
      {mode === "manual" ? (
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full rounded-[14px] bg-[#2563eb] px-4 py-3 text-[14px] font-medium text-white shadow-[0_8px_24px_rgba(37,99,235,0.35)]"
          >
            Importar CSV
          </button>
        </div>
...
function BottomNav({ active, onChange, isAdmin }) {
  ...
  return (
    <div className="mt-auto grid grid-cols-3 gap-0 rounded-[22px] border border-[#243041] bg-[#0f172a] p-1">
      {items.map((item) => {
